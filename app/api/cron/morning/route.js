...
import { sendMorningNudge }   from '../../../../lib/email'
...
export async function GET(request) {
  ...
  for (const [userId, userTasks] of Object.entries(byUser)) {
    try {
      ...
      await sendMorningNudge({ to: user.email, tasks: userTasks, goals, dayName })
      sent++
    } catch (e) { console.error(`Morning nudge failed for ${userId}:`, e) }
  }
